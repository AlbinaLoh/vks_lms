...
def news_create(request):
    # Проверяем, что пользователь аутентифицирован и является faculty
    if not request.user.is_authenticated or not request.user.faculty_id:
        messages.error(request, "У вас нет прав для создания новостей.")
        return redirect('news_list')

    faculty = get_object_or_404(Faculty, pk=request.user.faculty_id)
...
def news_edit(request, pk):
    ...
    if news.faculty.pk != request.user.faculty_id:
